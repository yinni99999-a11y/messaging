...
<details>
<summary>🛡️ 点击展开封禁功能使用说明</summary>

封禁功能允许管理员阻止特定用户使用机器人，被封禁用户发送的消息将被自动忽略。
...
如需使用群发功能中的`/post all`命令和用户封禁功能，需要配置KV存储来跟踪用户：
...
/ban 123456789            # 封禁指定ID的用户
/unban                    # 解封用户（回复消息或在话题中使用）
/unban 123456789          # 解封指定ID的用户

# 🗣️ 论坛模式操作（启用论坛模式时）
在用户话题中直接发送消息    # 直接回复给对应用户
回复话题中的消息           # 回复给对应用户
在用户话题中使用/ban       # 封禁该话题对应的用户
查看话题列表              # 每个话题对应一个用户
```
...
### v1.2.4 (Latest) - 🚫 用户封禁管理功能
- ✨ **新增封禁管理**: 支持封禁/解封用户，被封禁用户无法发送消息
- 🎯 **多种封禁方式**: 支持指定用户ID、回复消息、论坛话题内封禁
- 🔧 **智能识别**: 依次尝试从参数、回复消息、话题ID获取目标用户
- 🗣️ **论坛模式优化**: 在用户话题中直接使用 `/ban` 即可封禁该用户
- 🛠️ **条件判断优化**: 修复封禁命令的条件判断逻辑，确保所有方式都能正常工作
- 💾 **持久化存储**: 封禁状态保存在KV存储中，重启不丢失
- 🔐 **安全管理**: 仅管理员可以执行封禁/解封操作
